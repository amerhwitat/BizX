...
MANIFEST=STORE/'manifest.json'; PROVIDERS=json.loads(CFG.read_text())
UA='BizX-AssetBrowser/1.0 (+in-game asset browser)'
...
def search(provider,q,kind='image'):
 if provider=='openverse':
  base=PROVIDERS['openverse']['search_audio' if kind=='audio' else 'search_image']; u=base+'?'+urllib.parse.urlencode({'q':q,'page_size':24})
  data=json.loads(get(u)); out=[]
  for x in data.get('results',[]): out.append({'id':x.get('id'),'title':x.get('title'),'provider':'Openverse','type':kind,'license':x.get('license'),'license_url':x.get('license_url'),'creator':x.get('creator'),'source':x.get('foreign_landing_url') or x.get('url'),'preview':x.get('thumbnail') or x.get('url'),'download':x.get('url')})
  return out
 if provider=='polyhaven':
  data=json.loads(get(PROVIDERS['polyhaven']['assets'])); ql=q.lower(); out=[]
  for k,x in data.items():
   text=(k+' '+str(x)).lower()
   if not ql or ql in text: out.append({'id':k,'title':x.get('name',k),'provider':'Poly Haven','type':x.get('type','asset'),'license':'CC0','license_url':'https://polyhaven.com/license','source':'https://polyhaven.com/a/'+k,'preview':None,'download':None})
   if len(out)>=24: break
  return out
...
class H(http.server.BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  ...
  if p.path=='/api/v1/assets/search':
   q=urllib.parse.parse_qs(p.query); provider=q.get('provider',['openverse'])[0]; term=q.get('q',[''])[0]; kind=q.get('type',['image'])[0]
   try:return self.sendj({'results':search(provider,term,kind)})
 ...
 def do_POST(self):
  ...
  try:d=json.loads(self.rfile.read(int(self.headers.get('Content-Length','0')))); provider=d['provider']; url=d['download']; name=pathlib.Path(urlparse(url).path).name or (d.get('id','asset')+'.bin')
  except Exception:return self.sendj({'error':'invalid_json'},400)
  if provider not in PROVIDERS or not host_ok(url,provider): return self.sendj({'error':'download_host_not_allowed'},403)
  ext=pathlib.Path(name).suffix.lower()
  if ext and ext not in SAFE_EXT:return self.sendj({'error':'file_type_not_allowed'},415)
  try:data=get(url)
  ...
  digest=hashlib.sha256(data).hexdigest(); safe=''.join(c for c in name if c.isalnum() or c in '._-')[:160] or 'asset.bin'; dest=STORE/safe; dest.write_bytes(data)
  rec={'file':str(dest.relative_to(STORE)),'sha256':digest,'bytes':len(data),'provider':provider,'source':d.get('source'),'license':d.get('license'),'license_url':d.get('license_url'),'downloaded_at':__import__('datetime').datetime.now(__import__('datetime').timezone.utc).isoformat()}
  m=manifest(); m.append(rec); save_manifest(m); return self.sendj(rec)
