from bizx.monetization import MonetizationEngine, PAYMENT_ROUTING
from bizx.game.survival import PlayerResources
...
class TycoonGame:
    ...
    def advance_turn(self):
        revenue=sum(BUSINESSES[k]["revenue"]*count for k,count in self.businesses.items()); costs=sum(BUSINESSES[k]["costs"]*count for k,count in self.businesses.items()); self.cash+=revenue-costs; self.turn+=1; return {"turn":self.turn,"revenue":revenue,"costs":costs,"profit":revenue-costs,"cash":self.cash}
