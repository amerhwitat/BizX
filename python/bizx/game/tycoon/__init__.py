...
class TycoonGame:
    def __init__(self, cash=0, businesses=None):
        self.cash = cash
        self.businesses = dict(businesses or {})
        self.turn = 0

    def buy_business(self, business_type, price):
        if price <= 0 or self.cash < price or business_type not in BUSINESSES:
            return {"ok": False, "reason": "invalid-purchase"}
        self.cash -= price
        self.businesses[business_type] = self.businesses.get(business_type, 0) + 1
    ...
    def advance_turn(self):
        revenue = sum(BUSINESSES[k]["revenue"] * count for k, count in self.businesses.items())
        costs = sum(BUSINESSES[k]["costs"] * count for k, count in self.businesses.items())
        self.cash += revenue - costs
        self.turn += 1
        return {"turn": self.turn, "revenue": revenue, "costs": costs, "profit": revenue - costs, "cash": self.cash}

    def snapshot(self):
        return {"turn": self.turn, "cash": self.cash, "businesses": dict(self.businesses)}
