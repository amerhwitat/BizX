...
class ProgressionProfile:
    ...
    def save(self):
        return {"schema": 1, "name": self.name[:40], "level": self.level, "status": self.status, "score": self.score, "updatedAt": self.updated_at}
...
class HallOfFame:
    ...
    def submit(self, name="Player", score=0, game="default", level=0):
        ...
        entry={"name": str(name)[:40], "score": score, "game": game, "level": level, "recordedAt": datetime.now(timezone.utc).isoformat()}
        self.entries.append(entry); self.entries.sort(key=lambda e:e["score"], reverse=True); self.entries=self.entries[:self.limit]
        return {"eligible": True, "threshold": self.threshold, "entry": entry, "rank": self.entries.index(entry)+1}
