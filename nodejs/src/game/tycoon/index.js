...
export class TycoonGame {
  constructor({ cash = 0, businesses = {}, monetization = {}, playerResources = {} } = {}) {
    this.cash = cash; this.businesses = { ...businesses }; this.turn = 0; this.monetization = new MonetizationEngine(monetization); this.resources = new PlayerResources(playerResources);
  }
  buyBusiness(type, price) {
    ...
    this.cash -= price; this.businesses[type] = (this.businesses[type] ?? 0) + 1;
  ...
  advanceTurn() { let revenue = 0, costs = 0; for (const [type, count] of Object.entries(this.businesses)) { const definition = DEFAULT_BUSINESSES[type]; revenue += definition.revenue * count; costs += definition.costs * count; } this.cash += revenue - costs; this.turn += 1; return { turn: this.turn, revenue, costs, profit: revenue - costs, cash: this.cash }; }
