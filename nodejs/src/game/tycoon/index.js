...
export class TycoonGame {
  ...
  buyBusiness(type, price) {
    if (!Number.isFinite(price) || price <= 0 || this.cash < price) {
      return { ok: false, reason: 'insufficient-funds-or-invalid-price' };
    }
    const definition = DEFAULT_BUSINESSES[type];
    ...
    return {
      ok: true,
      business: type,
      payment: { asset: 'ETH', recipient: PRIMARY_ETH_ADDRESS, amount: price, status: 'wallet-authorization-required' }
    };
  }
  ...
  advanceTurn() {
    let revenue = 0;
    let costs = 0;
    for (const [type, count] of Object.entries(this.businesses)) {
      const definition = DEFAULT_BUSINESSES[type];
      revenue += definition.revenue * count;
      costs += definition.costs * count;
    }
  ...
  snapshot() {
    return { turn: this.turn, cash: this.cash, businesses: { ...this.businesses } };
  }
}
