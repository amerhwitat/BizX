...
export class TycoonGame {
  constructor({ cash = 0, businesses = {}, monetization = {} } = {}) {
    this.cash = cash;
    this.businesses = { ...businesses };
    this.turn = 0;
    this.monetization = new MonetizationEngine(monetization);
  }
  ...
  claimRewardedIncome(amount) {
    const result = this.monetization.grantRewardedAd('rewarded_double_income', { cash: amount });
    if (result.ok && Number.isFinite(amount) && amount > 0) this.cash += amount;
    return { ...result, cash: this.cash };
  }
  advanceTurn() {
    let revenue = 0, costs = 0;
    for (const [type, count] of Object.entries(this.businesses)) {
      const definition = DEFAULT_BUSINESSES[type];
      revenue += definition.revenue * count; costs += definition.costs * count;
    }
