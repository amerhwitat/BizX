...
**Goal:** Build the original Vanguard competitive tactical FPS, shared multiplayer stack, sandbox/voxel layer, original AAA asset/VFX/audio pipeline, and renderer/runtime adapters in BizX.
...
**Interfaces:**
- Consumes: existing BizX game/rendering/network conventions.
...
- Create: `tests/sandbox/voxel_test.cpp`
...
**Files:**
- Create/modify language-specific adapters under `cpp/`, `csharp/`, `rust/`, `java/`, `python/`, `node/`, `typescript/`, `kotlin/`, `swift/`, `flutter/`, `web/` according to the existing BizX structure.
