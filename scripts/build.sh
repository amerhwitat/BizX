...
pubspec.yaml) echo "== Dart: $d =="; (cd "$d"; dart pub get; [[ -d test ]] && dart test) || FAILURES=$((FAILURES+1));;
...
*.csproj|*.sln) echo "== C#/.NET: $d =="; (cd "$d"; dotnet restore; dotnet build --no-restore; [[ ! -d tests ]] || dotnet test --no-build) || FAILURES=$((FAILURES+1));;
...
echo "Build orchestration complete; failures=$FAILURES; log=$LOG" | tee -a "$LOG"; exit "$FAILURES"
