...
export async function scanPorts(target, ports, protocols, options = {}) {
  ...
  const interval = options.ratePerSecond ? Math.ceil(1000 / Math.min(options.ratePerSecond, 25)) : 0;
  const jobs = []; for (const port of ports) for (const protocol of protocols) jobs.push({ port, protocol });
  const out = []; let cursor = 0; let nextStart = 0;
  async function worker() {
    while (cursor < jobs.length) {
      const job = jobs[cursor++];
      if (interval) { const now=Date.now(); const wait=Math.max(0,nextStart-now); if(wait) await sleep(wait); nextStart=Math.max(Date.now(),nextStart)+interval; }
      out.push(await (job.protocol === 'tcp' ? probeTcp(target, job.port, options) : probeUdp(target, job.port, options)));
    }
  }
