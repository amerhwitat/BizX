...
public final class TycoonGame {
    ...
    private static final Map<String, Business> BUSINESSES = Map.of(
        "bakery", new Business("Bakery", 900, 300),
        "market", new Business("Market", 1500, 650),
        "factory", new Business("Factory", 2600, 1200));
    private long cash;
    private int turn;
    ...
    public PurchaseResult buyBusiness(String type, long price) {
        if (price <= 0 || cash < price) return new PurchaseResult(false, null);
        if (!BUSINESSES.containsKey(type)) return new PurchaseResult(false, null);
    ...
    public TurnResult advanceTurn() {
        long revenue = 0, costs = 0;
        for (var entry : owned.entrySet()) {
            Business b = BUSINESSES.get(entry.getKey());
            revenue += b.revenue() * entry.getValue(); costs += b.costs() * entry.getValue();
        }
        cash += revenue - costs; turn++;
        return new TurnResult(turn, revenue, costs, revenue - costs, cash);
    }
