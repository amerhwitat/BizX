...
public final class PlayerResources {
    ...
    public PurchasePrompt requestPurchase(String kind) {
        return "health".equals(kind) ? new PurchasePrompt("health", "medical_kit", 0.49, "Purchase a medical kit?") : new PurchasePrompt("ammo", "ammo_pack", 0.29, "Purchase an ammo pack?");
    }
    public boolean applyVerifiedPurchase(String productId) {
        switch (productId) { case "medical_kit" -> useMedicalKit(); case "ammo_pack" -> addAmmo(); default -> { return false; } }
        return true;
    }
